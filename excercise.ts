...
const getObject = <partialGet>(obj: partialGet) => Object.keys(obj) as (keyof partialGet)[];
...
return persons
    ...
    .filter((person) => {
        let criteriaKeys = getObject(criteria);
        return criteriaKeys.every((fieldName) => {
...
export function promisify<T>(fn: CallbackFunction<T>): PromiseFunction<T> {
...
type SourceObject<T> = {[K in keyof T]: CallbackFunction<T[K]>};
type PromisifiedObject<T> = {[K in keyof T]: PromiseFunction<T[K]>};
